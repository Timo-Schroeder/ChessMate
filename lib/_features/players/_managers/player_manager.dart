...
class PlayerManager extends SafeChangeNotifier {
  PlayerManager(this._databaseService);
  ...
  Future<void> fetchPlayers(int tournamentId) async {
    try {
      final fetchedPlayers = await _databaseService.getPlayersInTournament(
        tournamentId,
      );
      fetchedPlayers.fold(
        (error) => _players = [],
  ...
  Future<bool> addPlayer(int tournamentId) async {
    ...
    final result = await _databaseService.createPlayer(player);
    result.match(
      (error) =>
